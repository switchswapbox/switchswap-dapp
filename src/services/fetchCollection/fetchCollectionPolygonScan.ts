...
const fillCollection = (collection: INFTCollection, tokenId: string, from: string, to: string, walletaddress: string) => {
...
export async function getCollectionByAddress(walletaddress: string) {
  ...
  let params = {params: { module : 'account', action : 'tokennfttx', address : walletaddress , sort : 'desc'}}
  
  await axios.get(apiAdress, params)
  .then( (returnData: any) => {
